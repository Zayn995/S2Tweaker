# Startup repair for the withdrawn 1.37.0 release

The 1.37.0 GitHub release was withdrawn after a mostly black application
...
A subsequent portable build included the design polish.
...
This diagnostic is opt-in and excluded from automatic test selection.
The first measured run opened the real release runtime in 9.376 seconds at
...
native value displays and paged Loot & world controls were inspected using direct visual inspection. The window was fully drawn. That run closed automatically after
...
The measured repair build retained all 19
EXE/DLL/PYD files byte-for-byte from the withdrawn GitHub build. Only readable
Python sources changed. The portable headless selftest passed. This repair did not change game files
or installed mods.
