...
| Oodle | The game's pak lists Oodle as a compression method, but the entries this tool reads are stored **uncompressed** — measured against the real game: all 33 needed files, the three edition paks and the config entries of 11 third-party mod paks all extracted byte-identically with no Oodle library present. Since 1.19.1 the proprietary `oo2core_9_win64.dll` is therefore only fetched if repak actually fails for want of it, which in practice never happens. If a future game patch does compress that data, [pakio.py](s2tweaker/pakio.py) still obtains the library the careful way: local copies first, else one checksum-verified HTTPS download (repak would fetch it itself, but it validates TLS against its own built-in roots, which breaks behind AV/proxy HTTPS inspection). Packing never needs it.|
