"""Check magazine capacity precedence for weapon setups and linked attachments.

Individual weapon overrides precede categories and global values.
Derive expected values from installed data."""
import re
import sys
...
def mag_values(text: str) -> dict[str, int]:
    """Extract {item_SID: Magazine.MaxAmmo} from an item patch."""
    out = {}
...
# --- 1) Parameters and weapon-to-magazine mapping ---
...
assert len(orphans) <= 10, orphans          # Unique weapons without a list entry.
...
print(f"Zuordnung: {len(wm)} weapons, {len(used)}/{len(mags)} Magazine, "
      f"{len(orphans)} Waisen, {len(shared)} geteilte  OK")

# --- 2) Global x2: existing behavior across all magazines ---
...
# Check an individual AK74 override and its shared paired magazine.
...
print("Individual AK74 x2: 3 magazines + own MaxAmmo, nothing else  OK")

# Category factors affect rifle magazines, excluding pistol magazines.
...
print(f"Rifle category x2: {len(got)} magazines, pistols unchanged  OK")

# --- 5) Cascade: override > category > global ---
...
print("Cascade: AK74 x0.5 overrides rifle x2 and global x3  OK")

# --- 6) Neutral and lower bound ---
...
print("Neutral = no patch, minimum 1 round  OK")
