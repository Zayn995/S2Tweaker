"""Regression coverage for controls introduced in 1.26.0, using live baselines."""
import sys
...
print("Neutral: no 1.26.0 keys  OK")

# Player knockdown and water slowing.
...
print("Player: CanBeKnockedDown false, 3 water effects cleared  OK")

# --- 2) CoreVariables: ladder, look-down, zoom, A-Life visibility ---
...
# Retired ladder-view fields remain loadable but emit no patch.
...
assert core["HandlessFOVAimModifier"] == "0.2", core            # cap
...
# --- 4) EffectPrototypes: bash, guards, psy, teleports, clicker ---
...
print(f"Effects: bash 0, KillVolume cleared, psy phantoms, {len(tele)} instant teleports, clicker 35  OK")

# --- 5) ObjPrototypes toggles: anomalies and corpses ---
...
print(f"Obj-Schalter: immune species -> true, {len(obj.children)} human NPCs no longer loot  OK")

# Guard damage follows ordinary NPC parent weapon damage.
...
print("Guards: 32 GuardGun structs, AK 500 -> 9.5  OK")
...
print(f"Nachladen: {len(reload_rows)} magazine entries 1.0 -> 0.5, {len(jam_rows)} jam durations halved  OK")
...
assert "ChimeraLootGenerator" not in gen.children          # Already 1.0.
...
print("Trophies: Blinddog 0.15 -> 0.3, cap 1.0, Chimera unchanged  OK")
...
print("Map: distances doubled, 23 regions visible  OK")
...
print("Fast travel: 14 guides NoLock/Partial, GuideDelay 120 -> 60  OK")
...
print("Protection caps: 90 -> 100 (capped), Strike 4.5 -> 6.75, carry-weight path unchanged  OK")
...
print("Weird-Artifacts: Flower 7200 -> 14400, Bolt 300 -> 600  OK")
...
print("Intro: E01_MQ01_PlayVideo start connection cleared  OK")

# Prevent traders buying weapons/armor.
...
print("Clicker: 20 -> 10, minimum 1  OK")
...
print("Summary: 29 lines present, neutral empty  OK")
