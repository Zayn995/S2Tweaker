"""Verify every display alias resolves to an installed weapon or armor item."""
import sys
...
# Skip edition aliases only when their DLC data is unavailable.
...
assert not missing_w, f"Orphaned weapon aliases: {missing_w}"
assert not missing_a, f"Orphaned armor aliases: {missing_a}"
...
# Samples: display, search and armor-label precedence.
...
print(f"Aliase: {n_alias_w}/{len(pw)} weapons, {n_alias_a}/{len(pa)} "
      f"armors named, no orphaned entries\nNAMES TEST OK")
