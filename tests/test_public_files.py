    ...
    def test_zip_builder_checks_both_inventories_before_writing(self):
        for private_source, private_player in [(True, False), (False, True), (False, False)]:
            with self.subTest(source=private_source, player=private_player), TemporaryDirectory() as folder:
                ...
                (root / "docs").mkdir()
                for name in ("ANIMATION_SYNC.md", "PLAYER_RECOVERY_CONTROLS.md"):
                    (root / "docs" / name).write_text("Current guide", encoding="utf-8")
                    tracked.append("docs/" + name)
                (app / "README.txt").write_text("Old instructions", encoding="utf-8")
                (app / "Companion guide.md").write_text("Old guide", encoding="utf-8")
                cache = internal / "__pycache__"
                cache.mkdir()
                (cache / "fixture.pyc").write_bytes(b"generated bytecode")
                ...
                with patch.object(make_release_zips, "REPO", root), \
                    ...
                    if private_source or private_player:
                        ...
                        with zipfile.ZipFile(root / "release/S2Tweaker_vtest.zip") as archive:
                            ...
                            self.assertEqual(archive.read("README.txt"), b"Player instructions")
                            self.assertEqual(archive.read("Companion guide.md"), b"Current guide")
                            self.assertEqual(archive.read("Recovery controls.md"), b"Current guide")
                            self.assertNotIn("_internal/__pycache__/fixture.pyc", archive.namelist())
